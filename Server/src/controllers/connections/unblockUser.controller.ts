...
import { unblockUser } from "../../services/connections/unblockUser.service.js";
...
export const unblockUserController = async (req: Request, res: Response) => {
  try {
    ...
    const request = await unblockUser({ blockedId, blockerId });

    return successResponse(res, request, "Unblocked.", OK);
  } catch (error) {
